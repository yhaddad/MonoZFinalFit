...
parser.add_argument('-era', '--era', type=int, default=2017, help="")
parser.add_argument('-cfg', '--cfg', type=str, default="./config/inputs-NanoAODv4-tchain.yaml", help="")
parser.add_argument('-eos', '--eos', type=str, default="/eos/cms/store/group/phys_exotica/monoZ/LionKing2017/")
...
systematics_sources = [
 #"ElectronEn", "MuonEn", "jesTotal", "jer", "unclustEn",
 #"puWeight", "PDF", "MuonSF", "ElecronSF", "EWK", "nvtxWeight","TriggerSFWeight","btagEventWeight"
 "puWeight"
]
...
def drawing(channel="_3L", ylog=True, lumi=41.5, blind=True):
    ...
    for procname, cmd  in processes.items():
        print "we process this one : ", procname
        files     = cmd["files"]
        hist_objs = []
        dic_root_histos = {}
        for sample in files:
            for fn in list(glob.glob(os.path.join(options.eos, sample, '*.root'))):
                print " --- ", fn
    """
        ...
        #print("     --- %5s: %1.3f [events]" % (procname, hist_com.Integral()) )
